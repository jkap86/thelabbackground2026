...
export const updateKtcDataCurrent = async () => {
    ...
    const [allplayersArray, { ktc_map_dynasty, ktc_unmatched_dynasty }] = await Promise.all([getAllplayers(), getKtcMapAndUnmatched()]);
    const allplayers = Object.fromEntries(allplayersArray.data.map((player) => [player.player_id, player]));
    ...
    await upsertIntoCommon("ktc_map_dynasty", ktc_map_dynasty, new Date());
    await upsertIntoCommon("ktc_unmatched_dynasty", ktc_unmatched_dynasty, new Date());
    console.log(currentValues.length + " KTC dynasty current values to upsert...");
...
export const getKtcLinksToUpdate = async () => {
    ...
    const linksToUpdate = Object.keys(ktc_map_dynasty).filter((link) => !ktc_map_dynasty[link].sync || ktc_map_dynasty[link].sync < controlValue);
...
const matchPlayer = (player, allplayers, ktc_map) => {
    ...
    let matches = Object.keys(allplayers).filter((sleeper_id) => {
        ...
        const name_check = getMatchName(player.playerName).startsWith(getMatchName(allplayers[sleeper_id]?.first_name.slice(0, 3))) &&
            getMatchName(player.playerName).includes(getMatchName(allplayers[sleeper_id]?.last_name));
        return positon_check && name_check;
    });
    if (matches.length > 1) {
        matches = matches.filter((sleeper_id) => convertTeamAbbrev(player.team) === allplayers[sleeper_id]?.team);
    }
    if (matches.length === 1) {
...
const convertTeamAbbrev = (ktcTeam) => {
    const teamMap = {
        KCC: "KC",
        LVR: "LV",
        JAC: "JAX",
...
const upsertIntoCommon = async (field, data, updated_at) => {
    await pool.query(`
...
const upsertKtcValues = async (data) => {
    if (data.length === 0)
        return;
    const query = `
    ...
    if (values.length !== data.length * 5) {
        console.error(`Values mismatch: expected ${data.length * 5}, got ${values.length}`);
        console.error("Sample data item:", data[0]);
        return;
    }
    await pool.query(query, values);
};
const updatePlayerKtcHistory = async (link, sleeper_id) => {
