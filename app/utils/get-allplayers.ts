...
export async function getAllplayersEtagInfo() {
  const result = await pool.query(
    "SELECT updated_at FROM common WHERE name = 'allplayers'"
  );

  const updatedAt = result.rows[0]?.updated_at as Date | null;
...
export async function getAllplayersCached(): Promise<Allplayer[] | null> {
  const result = await pool.query(
    "SELECT data FROM common WHERE name = 'allplayers'"
  );
  return result.rows[0]?.data ?? null;
}

// Fetch fresh data from Sleeper API and update DB
export async function refreshAllplayers(): Promise<Allplayer[]> {
  const response: { data: { [player_id: string]: Allplayer } } =
  ...
  Object.values(response.data)
    ...
    .forEach((player) => {
      ...
      allplayers.push({
        ...
        last_name,
        age,
        fantasy_positions: fantasy_positions.map((pos) =>
          pos === "FB" ? "RB" : pos
        ),
        years_exp,
        active,
